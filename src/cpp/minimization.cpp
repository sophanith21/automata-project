...
int main()
{
    ...
    std::optional<FiniteAutomaton> min_dfa_opt; // Declare earlier
    FiniteAutomaton min_dfa;                    // Declare in outer scope
    bool has_min_dfa = false;
    if (original_fa.getType() == "DFA" && toMinimize)
    {
        ...
        if (min_dfa_opt)
        {
            ...
            {
                std::cerr << "\n--- Testing Converted DFA ---" << std::endl; // Changed to cerr
                std::string test_str1 = "aaaa";
                std::string test_str2 = "ab";
                std::string test_str3 = "b";
                std::string test_str4 = "aaabb";

                std::cerr << "Test '" << test_str1 << "': " << (min_dfa.testInput(test_str1) ? "Accepted" : "Rejected") << std::endl; // Changed to cerr
                std::cerr << "Test '" << test_str2 << "': " << (min_dfa.testInput(test_str2) ? "Accepted" : "Rejected") << std::endl; // Changed to cerr
                std::cerr << "Test '" << test_str3 << "': " << (min_dfa.testInput(test_str3) ? "Accepted" : "Rejected") << std::endl; // Changed to cerr
                std::cerr << "Test '" << test_str4 << "': " << (min_dfa.testInput(test_str4) ? "Accepted" : "Rejected") << std::endl; // Changed to cerr
            }
        }
        else
        {
